...
// A small strip above the hub cards showing the last 2-3 services a user tapped.
function RecentlyViewed({ onSelect }: { onSelect: (svc: SelectedService) => void }) {
  const [recent, setRecent] = useState<SelectedService[]>([])

  useEffect(() => {
    const data = sessionStorage.getItem("abh-recent-services")
      ...
      <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
        {recent.map((svc, i) => (
          <button
            ...
            className="flex-shrink-0 flex items-center gap-3 px-4 py-3 rounded-[14px] bg-white dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 shadow-sm hover:shadow-md transition-all active:scale-95"
          >
            <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-zinc-100 dark:bg-zinc-900">
              <HubIcon id={svc.hubId} size={18} color={HUB_COLORS[svc.hubId as HubKey].tagText} />
            </div>
            <div className="text-left">
              <p className="text-xs font-black text-zinc-800 dark:text-zinc-200">{svc.name}</p>
              <p className="text-[0.6rem] font-bold text-zinc-400 uppercase tracking-tight">{svc.price}</p>
            </div>
          </button>
        ))}
      </div>
...
function HubModal({
  ...
  // Smooth section scroll: auto-scroll the pill into view so it's always visible.
  useEffect(() => {
    if (openSectionIdx !== null && pillContainerRef.current) {
      const pill = pillContainerRef.current.children[openSectionIdx] as HTMLElement
      if (pill) pill.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' })
    }
  }, [openSectionIdx])

  // Escape closes this modal — matches ZoomOverlay behaviour
  useEffect(() => {
    const fn = (e: KeyboardEvent) => {
      // Fix: Add keydown listener to each, closing the innermost one first
      if (e.key === "Escape" && !document.querySelector('[data-is-service-modal="true"]')) {
        onClose()
      }
    }
  ...
  // "Most popular" badge on 2-3 services per hub
  const isPopular = (name: string) => ["Black & White", "Colour", "CV Revamp", "Online Application", "Premium Logo"].some(p => name.includes(p))
    ...
    <div className="fixed inset-0 z-[10100] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-zinc-950 rounded-[14px] overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-500 border border-zinc-100 dark:border-zinc-800">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-6 md:p-8">
          <div ref={pillContainerRef} className="inline-flex flex-nowrap md:flex-wrap gap-2 mb-5 overflow-x-auto no-scrollbar pb-1">
            {hub.sections.map((section, sIdx) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "px-3.5 py-1.5 rounded-full text-[0.7rem] font-black tracking-tight whitespace-nowrap transition-all duration-200 shrink-0",
              ...
              {hub.sections[openSectionIdx].items.map((item, iIdx) => (
                ...
                >
                  <div className="flex flex-col items-start gap-1">
                    ...
                    {isPopular(item.name) && (
                      <span className="flex items-center gap-1 text-[0.6rem] font-black uppercase tracking-tighter px-1.5 py-0.5 rounded-md bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400">
...
// Turnaround times keyed by section title, with item-level overrides
...
function ServiceDetailModal({ svc, onClose }: { svc: SelectedService | null; onClose: () => void }) {
  ...
  useEffect(() => { setTab("bring"); setAddedToQuote(false) }, [svc?.name])

  // Escape closes the service detail, leaving the hub modal open behind it
  useEffect(() => {
    if (!svc) return
  ...
  return (
    <div className="fixed inset-0 z-[10200] flex items-center justify-center p-4 animate-in fade-in duration-300" data-is-service-modal="true">
      ...
      <div className="relative w-full max-w-sm rounded-[14px] overflow-hidden shadow-2xl bg-white dark:bg-zinc-950 animate-in zoom-in-95 duration-300 border border-zinc-100 dark:border-zinc-800 max-h-[88vh] flex flex-col">
        ...
        <div className="p-6 pb-0 flex-shrink-0">
          ...
          <div className="flex items-center justify-between gap-3 mb-5">
            ...
            <span className="flex items-center gap-1.5 text-[0.65rem] font-bold text-zinc-400 dark:text-zinc-500 whitespace-nowrap">
          ...
          <div className="flex gap-2">
            {(["bring", "about"] as Tab[]).map((t) => (
              ...
              >
                {t === "bring" ? "What to Bring" : "About Service"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="px-6 pb-6 pt-3 flex-shrink-0 border-t border-zinc-100 dark:border-zinc-800 space-y-2.5">
          {/* Add to Quote — fires a custom DOM event the QuoteCalculatorWidget listens for */}
          <button
            type="button"
            onClick={() => {
            ...
            className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-[14px] font-black text-sm border-2 transition-all active:scale-95"
            style={addedToQuote
...
export function ServicesPage() {
  ...
  // Handle service selection and update recently viewed
  const handleSelectService = useCallback((svc: SelectedService) => {
    setSelectedService(svc)
    const data = sessionStorage.getItem("abh-recent-services")
    let list: SelectedService[] = data ? JSON.parse(data) : []
    // Add to top, remove duplicates, limit to 5
    list = [svc, ...list.filter(item => item.name !== svc.name)].slice(0, 5)
    sessionStorage.setItem("abh-recent-services", JSON.stringify(list))
  }, [])
  ...
  return (
    <section className="min-h-screen bg-background pt-[calc(var(--nav-h)+2rem)] pb-24 px-4 md:px-8">
      ...
      <FloatingSearchPill onSelect={handleSelectService} visible={pillVisible} />
      ...
      <div className="max-w-[1300px] mx-auto">
        ...
        {/* Recently viewed services strip */}
        <RecentlyViewed onSelect={handleSelectService} />
        ...
        <div className="flex flex-col md:flex-row gap-6 pb-8 overflow-x-auto md:overflow-visible no-scrollbar">
          {HUB_ORDER.map((hubId) => {
            ...
            const colors = HUB_COLORS[hubId as HubKey]
            const accent = isDark ? colors.tagTextDark : colors.tagText
            const serviceCount = hub.sections.reduce((acc, s) => acc + s.items.length, 0)
            ...
            return (
              ...
              >
                ...
                <h3 className="font-sans font-black text-xl text-zinc-900 dark:text-zinc-50 mb-1 group-hover:text-brand-blue">
                  {hub.title}
                </h3>
                {/* Service count in the hub card */}
                <p className="text-[0.65rem] font-bold uppercase tracking-widest text-zinc-400 mb-3">{serviceCount} services</p>
