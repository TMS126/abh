                  ...
                  <div className="px-4 py-3 bg-background">
                    <div className="flex flex-wrap gap-1.5">
                      {section.items.map((item, itemIdx) => (
                        <span
                          key={itemIdx}
                          className="group relative inline-flex items-center px-3 py-1.5 rounded-2xl text-[0.76rem] font-semibold font-sans cursor-default transition-all duration-200 ease-in-out hover:-translate-y-0.5 active:scale-95"
                          style={{ background: tagStyle.bg, color: tagStyle.color }}
                        >
                          {item.name}
                          <span
                            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2.5 px-3 py-1.5 rounded-lg text-[0.78rem] font-extrabold whitespace-nowrap opacity-0 pointer-events-none transition-all duration-200 ease-in-out scale-90 group-hover:opacity-100 group-hover:scale-100 shadow-lg z-10"
                            style={{ background: tagStyle.color, color: tagStyle.bg }}
                          >
                            {item.price}
                            <span
                              className="absolute top-full left-1/2 -translate-x-1/2 border-[5px] border-transparent"
                              style={{ borderTopColor: tagStyle.color }}
                            />
                          </span>
                        </span>
                      ))}
  ...
  return (
    ...
    >
      <div className="px-5 py-4 flex items-center gap-3" style={{ background: hub.grad }}>
        ...
        <span className="ml-auto w-[30px] h-[30px] bg-white/20 rounded-full flex items-center justify-center text-white text-sm shrink-0 transition-all duration-300 ease-in-out">
        ...
        <section className="px-4 md:px-8 py-12 md:py-16">
          <div className="max-w-[680px] mx-auto mb-8 md:mb-10 text-center">
            <p className="text-muted-foreground text-[0.95rem] leading-relaxed">
              {"Tap any hub card to explore all services. Hover or tap a service tag to reveal its price."}
            </p>
          </div>
