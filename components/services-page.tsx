...
function FloatingSearchPill({
  ...
  // Three hub accent colours cycling on the icon when collapsed
  const CYCLE = ["#1E6FA8", "#3E6B0E", "#B86F34"]
  useEffect(() => {
                ...
                return (
                  ...
                  >
                    <div className="w-9 h-9 rounded-[10px] flex items-center justify-center shrink-0" style={{ backgroundColor: `${accent}15`, color: accent }}>
                      <HubIcon id={s.hubId} size={17} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-black text-zinc-800 dark:text-zinc-200 truncate">{s.name}</p>
                      <p className="text-[0.62rem] font-medium text-zinc-400 truncate">{s.sectionTitle} · {HUBS[s.hubId].title}</p>
                    </div>
