...
export function StatsBar() {
  ...
  return (
    <section aria-label="Key stats" className="px-4 md:px-8 py-8 md:py-10 transition-colors duration-300">
      <div className="grid grid-cols-3 gap-3 sm:gap-4 w-full max-w-[560px] mx-auto" role="list" aria-label="Key stats">
        {stats.map((stat, i) => {
          ...
          return (
            <ScrollBounce key={stat.label} delay={i * 0.08}>
              ...
              >
                <Icon size={22} weight="fill" aria-hidden="true" style={{ color: stat.color }} className="mb-0.5" />
                <div className="text-xl font-black text-zinc-900 dark:text-zinc-50">{stat.value}</div>
                <div className="text-[0.72rem] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500">{stat.label}</div>
              </div>
            </ScrollBounce>
          )
        })}
