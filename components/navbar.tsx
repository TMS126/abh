...
export function Navbar({ activePage }: NavbarProps) {
  ...
  return (
    <>
      {/* ── 1. FIXED TOP ACTION BAR BUTTON ROW ── */}
      <div
      ...
      >
        <div className="flex items-center justify-between w-full max-w-[1200px]">
          ...
          <div className="flex items-center gap-3 pointer-events-auto ml-auto">
            ...
            <div className="flex items-center gap-2 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800 shadow-sm h-11">
              ...
              <button
                ...
                className={`w-[36px] h-[18px] rounded-full relative transition-colors duration-300 flex items-center p-[2px] ${
                  theme === "dark" ? "bg-[#1E6FA8]" : "bg-gray-200 dark:bg-zinc-700"
                }`}
            ...
            {/* Menu Trigger Action Node */}
            <button
              onClick={() => setMenuOpen(!menuOpen)}
              className={`w-11 h-11 rounded-full flex flex-col items-center justify-center gap-[4.5px] transition-all duration-300 active:scale-95 border ${
                menuOpen
                  ? "bg-[#1E6FA8] border-[#0f3f66] text-white rotate-90"
                  : "bg-[#F4A261] border-[#D9894B] text-white hover:bg-[#D9894B] shadow-[0_4px_14px_rgba(244,162,97,0.35)]"
              }`}
      ...
      {/* ── 2. FULLSCREEN/OVERLAY LINK MODULE WORKSPACE ── */}
      <div
      ...
      >
        {/* Semi-Transparent Background Mask with Controlled Desktop Blur Layer */}
        <div
          className="absolute inset-0 backdrop-blur-md transition-opacity duration-300"
          style={{
            background: theme === "dark" ? "rgba(10, 26, 46, 0.72)" : "rgba(255, 255, 255, 0.78)",
          }}
          onClick={() => setMenuOpen(false)}
        />

        {/* Header Clear Target Area */}
        <div className="h-[80px] w-full shrink-0" />

        {/* Global Nav Center Array — Shifted Higher on Desktop Screens */}
        <nav className="relative z-10 w-full max-w-[1000px] flex flex-col items-center md:justify-start justify-center grow md:pt-10">
          
          {/* THE LINK HOUSE: Self-contained rounded bounding container element */}
          <div 
            className={`p-5 md:p-4 rounded-[24px] border border-gray-200/60 dark:border-zinc-800/60 bg-white/60 dark:bg-zinc-950/40 backdrop-blur-md shadow-xl flex flex-col md:flex-row items-center justify-center gap-3 w-full md:w-auto max-w-sm md:max-w-none transition-all duration-300 transform ${
              ...
              return (
                <button
                  ...
                  className={`py-3.5 px-6 md:py-2.5 md:px-5 rounded-[16px] font-sans font-black text-sm transition-all duration-300 active:scale-95 text-center shrink-0 w-full md:w-auto transform ${
                    ...
                    item.isCta
                      ? "bg-[#F4A261] text-white hover:bg-[#D9894B] shadow-sm"
                      : isActive
                        ? "bg-[#1E6FA8] text-white"
                        : "bg-gray-100/80 dark:bg-zinc-900/80 text-gray-800 dark:text-zinc-200 hover:bg-gray-200 dark:hover:bg-zinc-800"
                  }`}
                  style={{
        ...
        {/* Monochromatic Mini Footer Signature Frame */}
        <div
          ...
          style={{ transitionDelay: menuOpen ? `${navItems.length * 35 + 50}ms` : "0ms" }}
        >
          {/* Monochromatic Grayscale Branding */}
          <div className="font-sans font-black text-sm tracking-tight text-gray-500 dark:text-zinc-400">
            ApexbytesHub
          </div>
          {/* Customized Founder Tagline Integration */}
          <p className="text-gray-400 dark:text-zinc-500 text-[0.68rem] font-bold tracking-widest mt-1 uppercase">
            founded by Theji M
          </p>
        </div>
      </div>
