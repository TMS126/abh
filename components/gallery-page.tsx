      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row gap-2 justify-center mb-16">
          ...
          {ROW_ORDER.map(row => {
            ...
            return <button key={row.id} onClick={() => setActiveFilter(row.id)} className={cn("px-6 py-2.5 rounded-full text-xs font-black uppercase tracking-widest transition-all", isActive ? cn("shadow-lg", isDark ? "text-zinc-900" : "text-white") : "bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-800")} style={isActive ? { backgroundColor: accent } : {}}>{row.short}</button>
          })}
        </div>
