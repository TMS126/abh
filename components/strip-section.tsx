...
export function StripSection() {
  const items = [
    ...
    { icon: "📍", title: "Home-Based", desc: "Kgotsong, Bothaville 9660" },
  ]
      ...
      <div className="max-w-[1080px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {items.map((item) => (
          <div key={item.title} className="flex items-center gap-4">
            <div className="w-[50px] h-[50px] bg-card rounded-[13px] flex items-center justify-center text-2xl shadow-[var(--shadow)] shrink-0">
              {item.icon}
            </div>
            <div>
              <h4 className="font-sans font-bold text-blue-3 dark:text-blue-4 text-[0.92rem]">
                {item.title}
              </h4>
              <p className="text-muted-foreground text-[0.78rem] mt-0.5">
                {item.desc}
              </p>
            </div>
          </div>
    ...
    <section className="bg-gradient-to-r from-green-3 to-green-1 py-10 px-4 md:px-8 text-center text-white">
      ...
      {buttonHref ? (
        <a
          ...
          className="inline-flex items-center gap-2 px-7 py-3 rounded-[28px] font-sans font-extrabold text-[0.95rem] bg-wa-green text-white hover:bg-[#1ebe5a] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_28px_rgba(37,211,102,0.4)]"
        >
          {buttonText}
        </a>
      ) : (
