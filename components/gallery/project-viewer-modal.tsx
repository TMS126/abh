...
// How much scroll (px) it takes before the title fully "jumps" up into
// the image area. Same distance used to drive the desktop header shrink.
const HEADER_SHRINK_DISTANCE = 70

// Short badge text for the client-type pill (now compact, not a sentence).
const CLIENT_TYPE_TEXT: Record<string, string> = {
  client: "Client work",
...
// Minimal CTAs — plain text+icon, no filled/bordered boxes. Unchanged —
// this was already the "no pill bg" style.
...
export function ProjectViewerModal({
  ...
  // Once fully scrolled, the title "jumps" into the image area; scrolling
  // back to the top brings it back down to its normal spot below the image.
  const titleInImage = scrollProgress >= 1
  ...
  const clientTypeText = project.clientType ? CLIENT_TYPE_TEXT[project.clientType] : null
        ...
        {isMobile ? (
          <>
            <div className="relative h-[38%] shrink-0 overflow-hidden bg-zinc-900">
              {comparing && hasBA ? (
                ...
                >
                  ...
                  {/* Top row: single hub icon, centered, with shadow — Close, top-right */}
                  <div className="absolute top-0 inset-x-0 z-20 flex items-center justify-between px-3 pt-3 pb-8 bg-gradient-to-b from-black/50 to-transparent pointer-events-none">
                    ...
                    <div
                      ...
                      style={{ backgroundColor: accent, boxShadow: "0 6px 16px -4px rgba(0,0,0,0.55), 0 2px 6px -2px rgba(0,0,0,0.35)" }}
                    >
                      <HubIcon id={project.hub as HubId} size={16} color="#ffffff" />
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); onClose() }}
                      aria-label="Close"
                      className={cn("pointer-events-auto shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-white transition-transform active:scale-90", CHIP)}
                    >
                      <X size={17} weight="bold" />
                    </button>
                  </div>

                  {/* Title jumps up here once scrolled past the threshold */}
                  {titleInImage && (
                    <div className="absolute inset-x-0 top-14 z-20 px-6 pointer-events-none animate-in fade-in duration-150">
                      <h2 className="font-black text-[0.98rem] text-white text-center leading-snug [text-shadow:0_1px_6px_rgba(0,0,0,0.6)]">
                        {project.title}
                      </h2>
                    </div>
                  )}

                  {/* Combined row: client-type badge (flush left) + Like/Share (horizontal, right) */}
                  <div className="absolute bottom-16 inset-x-0 z-20 flex items-center justify-between px-3">
                    {clientTypeText ? (
                      <span className={cn("px-2.5 py-1 rounded-full text-[0.7rem] font-bold whitespace-nowrap", CHIP, project.clientType === "sample" && "text-brand-orange")}>
                        {clientTypeText}
                      </span>
                    ) : <span />}
            ...
            <div ref={detailsRef} onScroll={handleDetailsScroll} className="flex-1 overflow-y-auto overscroll-contain">
              {/* Title — no background/pill at all now. Only shown here
                  while it hasn't jumped up into the image; content simply
                  scrolls up past/behind it since there's no backdrop. */}
              {!titleInImage && (
                <div className="sticky top-0 z-20 px-6 pt-5 pb-3 bg-white dark:bg-zinc-950 animate-in fade-in duration-150">
                  <h2 className="font-black text-base text-zinc-900 dark:text-zinc-50 text-center leading-snug">
                    {project.title}
                  </h2>
                  <div className="mt-2">
          ...
          <>
              ...
              {comparing && hasBA ? (
                ...
                <>
                  <div className="relative flex-1 overflow-hidden cursor-zoom-in group/img" onClick={handleImageClick}>
                    ...
                    {/* Single hub icon, centered, with shadow — replaces the hub-filter row */}
                    <div
            ...
            <div className="relative flex flex-col border-zinc-100 dark:border-zinc-800 flex-1 border-t md:h-auto md:flex-none md:border-t-0 md:border-l md:w-[380px]">
              <div className="shrink-0 px-6 md:px-8 pt-6 md:pt-8 pb-5 relative z-20">
                ...
                <h2 className="font-sans font-black text-xl text-zinc-900 dark:text-zinc-50 leading-snug mt-2">
                  {project.title}
                </h2>
                {project.clientType && (
                  <p className="text-[0.86rem] italic mt-2">
