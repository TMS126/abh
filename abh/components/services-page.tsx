...
function ServiceDetailModal({ svc, onClose }: { svc: SelectedService | null; onClose: () => void }) {
  ...
  const accent = colors.tagTextDark   // vibrant on dark bg
  ...
  return (
    <div className="fixed inset-0 z-[10200] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div
        ref={ref}
        className="relative w-full max-w-sm rounded-[14px] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border border-white/10"
      ...
      >
        <div className="p-8">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <p className="text-[0.6rem] font-black uppercase tracking-[0.18em] mb-2" style={{ color: `${accent}70` }}>
                {svc.sectionTitle} — {svc.name}
              </p>
          ...
          <p className="text-sm leading-relaxed mb-8 font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>{desc}</p>
          ...
          <a
            ...
            className="flex items-center justify-center gap-3 w-full py-4 rounded-[14px] font-black text-sm text-white transition-all active:scale-95 hover:opacity-90"
...
export function ServicesPage() {
  ...
  function flipTo(hubId: HubId) { setFlippedHub(hubId); setBackSection(0) }
  function flipBack()            { setFlippedHub(null) }
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          {HUB_ORDER.map(hubId => {
            ...
            return (
              <div
                key={hubId}
                className="relative rounded-[14px] border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 overflow-hidden"
                style={{ minHeight: "360px" }}
              >
                {/* ── FRONT FACE ── */}
                <div className={cn(
                  "absolute inset-0 flex flex-col items-center p-8 text-center transition-all duration-300",
                  isFlipped ? "opacity-0 pointer-events-none -translate-y-2 scale-95" : "opacity-100 translate-y-0 scale-100"
                )}>
                  ...
                  <div className="mt-auto flex flex-col items-center gap-4">
                    ...
                    <button
                      ...
                      className="text-[0.65rem] font-black uppercase tracking-widest transition-opacity hover:opacity-60"
                      style={{ color: accent }}
                    >
                      View Services →
                    </button>
                ...
                <div className={cn(
                  ...
                  isFlipped ? "opacity-100 translate-y-0 scale-100" : "opacity-0 pointer-events-none translate-y-2 scale-95"
                )}>
                  {/* Header — click to flip back */}
                  <button
                    onClick={flipBack}
                    className="flex items-center gap-2 px-4 py-3.5 border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors shrink-0 text-left"
                  >
                    <CaretLeft size={13} weight="bold" style={{ color: accent }} />
                    <span className="font-black text-[0.7rem] uppercase tracking-widest" style={{ color: accent }}>{hub.title}</span>
                  </button>

                  {/* Sections accordion */}
                  <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
                    {hub.sections.map((section, sIdx) => {
                      const isOpen = backSection === sIdx
                      return (
                        <div
                          key={sIdx}
                          className={cn(
                            "rounded-[14px] border transition-all duration-300 origin-top",
                            isOpen
                        ...
                        >
                          <button
                            onClick={() => setBackSection(sIdx)}
                            className="w-full flex items-center justify-between px-3.5 py-3 text-left"
                          >
                            <span className={cn("font-black text-[0.72rem] tracking-tight", isOpen ? "text-zinc-900 dark:text-zinc-50" : "text-zinc-500")}>
                              {section.title}
                            </span>
                            <CaretDown size={11} className={cn("shrink-0 transition-transform duration-300", isOpen && "rotate-180")} style={{ color: isOpen ? accent : undefined }} />
                          </button>
                          <Accordion open={isOpen}>
                            <div className="px-3.5 pb-3.5 flex flex-wrap gap-1.5">
                              {section.items.map((item, iIdx) => (
                                <button
                                  ...
                                  className="px-3 py-1.5 rounded-[14px] border text-[0.72rem] font-bold transition-all hover:-translate-y-0.5 active:scale-95"
